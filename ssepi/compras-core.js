// taller-core.js - SSEPI Laboratorio de Electrónica V20
// COMPLETO - Con guardado automático, fotos opcionales y vinculación con compras
// AHORA CON NOTIFICACIONES EN TIEMPO REAL
...
const TallerCore = (function() {
    ...
    let p_notificaciones = [];        // notificaciones para taller
    ...
    function _cargarNotificaciones() {
        ...
        // Escuchar notificaciones de compras (NO LEÍDAS)
        window.db.collection('notificaciones')
            .where('para', '==', 'taller')
            .where('leido', '==', false)
            .onSnapshot(snap => {
                p_notificaciones = snap.docs.map(d => ({ id: d.id, ...d.data() }));
                const notificaciones = p_notificaciones.length;
                if (notificaciones > 0) {
                    _mostrarNotificacionesRecientes();
                    _actualizarBadgeNotificaciones(notificaciones);
                } else {
                    _actualizarBadgeNotificaciones(0);
                }
            });
    }

    function _mostrarNotificacionesRecientes() {
        // Mostrar las 3 más recientes no leídas
        const recientes = p_notificaciones.slice(0, 3);
        recientes.forEach(notif => {
            _mostrarNotificacion(notif);
        });
    }

    function _mostrarNotificacion(notificacion) {
        ...
        let icono = '📦';
        if (notificacion.tipo === 'material_recibido') icono = '📥';
        if (notificacion.tipo === 'material_entregado') icono = '✅';
        
        notif.innerHTML = `
            <div style="font-weight:800; margin-bottom:5px;">${icono} ${notificacion.tipo === 'material_recibido' ? 'MATERIAL RECIBIDO' : 'MATERIAL ENTREGADO'}</div>
            <div style="font-size:13px;">${notificacion.mensaje}</div>
            <div style="font-size:10px; margin-top:5px; opacity:0.8;">${new Date(notificacion.fecha).toLocaleTimeString()}</div>
        `;
        notif.onclick = () => {
            notif.remove();
            // Marcar como leída
            if (window.db) {
                window.db.collection('notificaciones').doc(notificacion.id).update({ leido: true });
            }
            // Si tiene ordenTallerId, abrir esa orden
            if (notificacion.ordenTallerId) {
                _abrirOrden(notificacion.ordenTallerId);
            }
        };
                    ...
                    if (tallerId) {
                        ...
                        if (compra.estado === 5 && change.type === 'modified') {
                            _mostrarNotificacion({
                                ...
                                mensaje: `✅ Materiales de orden ${compra.folio} entregados a taller`,
                                fecha: new Date().toISOString(),
                                ordenTallerId: tallerId
                            });
                        }
                    }
                });
