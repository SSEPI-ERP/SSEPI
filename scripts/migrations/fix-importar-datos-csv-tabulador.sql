-- ============================================================================
-- FIX IMPORTACIÓN: Datos del CSV TABULADOR DE COTIZACIÓN (1).csv
-- Tabla ya existe con columna 'cliente_nombre' (no 'nombre_cliente')
-- ============================================================================

-- 1. Asegurar columnas numéricas faltantes
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema='public' AND table_name='clientes_tabulador' AND column_name='km') THEN
        ALTER TABLE public.clientes_tabulador ADD COLUMN km NUMERIC(10,2) DEFAULT 0;
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema='public' AND table_name='clientes_tabulador' AND column_name='horas_viaje') THEN
        ALTER TABLE public.clientes_tabulador ADD COLUMN horas_viaje NUMERIC(5,2) DEFAULT 0;
    END IF;
    ...
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema='public' AND table_name='clientes_tabulador' AND column_name='creado_en') THEN
        ALTER TABLE public.clientes_tabulador ADD COLUMN creado_en TIMESTAMPTZ DEFAULT NOW();
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema='public' AND table_name='clientes_tabulador' AND column_name='actualizado_en') THEN
        ALTER TABLE public.clientes_tabulador ADD COLUMN actualizado_en TIMESTAMPTZ DEFAULT NOW();
    END IF;
END $$;

-- 2. Limpiar datos previos (TRUNCATE elimina TODOS los registros)
TRUNCATE public.clientes_tabulador RESTART IDENTITY;

-- 3. Insertar datos del CSV usando la columna REAL: cliente_nombre
INSERT INTO public.clientes_tabulador (
    cliente_nombre, km, horas_viaje, litros, precio_gasolina, costo_gasolina, hr_dani, costo_dani, total_viaje
) VALUES
