...
// DESCRIPCIÓN: Módulo completo de Contactos (Clientes y Proveedores)
// SEGURIDAD: Integración con servicios de datos, autenticación, auditoría
//            Encriptación de datos sensibles en BD (via triggers)
...
const ContactosModule = (function() {
    ...
    // Lista completa de contactos iniciales (para importación si está vacío)
    const CONTACTOS_INICIALES = [
        { nombre: 'Anguiplast, S.A. de C.V.', empresa: 'Anguiplast, S.A. de C.V.', telefono: '+52 348 784 6573', email: 'aluquin@anguiplast.com', direccion: 'Arandas, México', rfc: 'ANG101215PG0', sitio: 'http://www.anguiplast.com', tipo: 'client', avatar: 'A', color: '#2e7d32' },
        { nombre: 'Jaziel Lopez', empresa: 'Anguiplast, S.A. de C.V.', puesto: 'Ventas', telefono: '+52 348 784 6573', email: 'ventas@anguiplast.com', direccion: 'Libramiento Norte Km. 2\nArandas Centro\n47180 Arandas, JAL\nMéxico', rfc: 'ANG101215PG0', sitio: 'http://www.anguiplast.com.mx', tipo: 'client', avatar: 'J', color: '#4caf50' },
        ...
        { nombre: 'SSEPI', empresa: 'SSEPI', email: 'administracion@ssepi.org', direccion: 'Leon, México', sitio: 'http://www.ssepi.org', tipo: 'provider', avatar: 'S', color: '#00a09d' },
    ...
    function _bindEvents() {
        ...
        document.getElementById('newContactBtn').addEventListener('click', () => _openModal('nuevo'));
        document.getElementById('importBtn').addEventListener('click', () => document.getElementById('fileInput').click());
        document.getElementById('fileInput').addEventListener('change', _handleCSVImport);
        document.getElementById('saveContactBtn').addEventListener('click', _saveContact);
        document.querySelector('.panel-close').addEventListener('click', _closeDetail);
        document.getElementById('backdrop').addEventListener('click', _closeDetail);
        document.getElementById('btnWhatsApp').addEventListener('click', _enviarWhatsApp);
        document.getElementById('updateContactBtn').addEventListener('click', _updateContactData);
        document.getElementById('panelLogoUrl').addEventListener('input', _updateAvatarPreview);
        document.getElementById('panelTipo').addEventListener('change', (e) => { if (contactoSeleccionado) contactoSeleccionado.tipo = e.target.value; });
    }
        ...
        container.innerHTML = contacts.map(c => {
            ...
            return `
                <div class="contact-card" data-id="${c.id}" onclick="contactosModule.abrirDetalle('${c.id}')">
                    <div class="avatar-box" style="${estiloAvatar}">${c.logoUrl ? '' : inicial}</div>
                    <div class="info">
    ...
    async function _updateKPIs() {
        ...
        try {
            ...
            if (!error) saldoTotal = data.reduce((s, i) => s + (i.monto_total || 0), 0);
            ...
            data.forEach(act => {
                ...
                html += `
                    <div class="timeline-item">
                        <div class="timeline-icon" style="background: var(--c-contact);"><i class="${icon}"></i></div>
