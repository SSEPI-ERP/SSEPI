...
def merge_client_lists(original_clients: List[Dict], enhanced_clients: List[Dict]) -> List[Dict]:
    """Merge the two client lists by grouping enhanced clients that match to the same original client"""
    # Create a mapping from original clients to lists of enhanced clients that match them
    original_to_enhanced_map: Dict[int, List[Dict]] = {id(orig): [] for orig in original_clients}

    # Also track which enhanced clients remain unmatched
    unmatched_enhanced: List[Dict] = []

    # Assign each enhanced client to its best matching original client
    for enhanced_client in enhanced_clients:
        ...
        if best_match:
            original_to_enhanced_map[id(best_match)].append(enhanced_client)
        else:
            unmatched_enhanced.append(enhanced_client)

    # Build the merged list
    merged_clients = []

    # Process each original client and its matched enhanced clients
    for original_client in original_clients:
        matched_enhanced = original_to_enhanced_map[id(original_client)]

        if matched_enhanced:
            # We have one or more enhanced clients that match this original client
            # We'll merge them all into one client
            # Use the first enhanced client as base, but we could also try to pick the "best" one
            base_enhanced = matched_enhanced[0]
            merged = base_enhanced.copy()

            # Override with original client data for RFC and c1 (KM) and other fields
            merged['rfc'] = original_client['rfc']
            merged['c1'] = float(original_client['c1'])
            # Also override other fields from original if they have meaningful values
            if original_client['c2'] != 0:
                merged['c2'] = float(original_client['c2'])
            if original_client['c3'] != 0:
                merged['c3'] = float(original_client['c3'])
            if original_client['c4'] != 0:
                merged['c4'] = float(original_client['c4'])
            if original_client['c5'] != 0:
                merged['c5'] = float(original_client['c5'])
            if original_client['c6'] != 0:
                merged['c6'] = float(original_client['c6'])
            if original_client['address']:
                merged['address'] = original_client['address']
            if original_client['contact']:
                merged['contact'] = original_client['contact']

            # For sheets, we could combine all sheets from all matched enhanced clients
            # But for now, let's just use the sheets from the first enhanced client
            # (In practice, they should be the same or similar)

        else:
            # No enhanced clients matched this original client
            # Create a basic client entry for the original
            merged = {
    ...
    # Add any unmatched enhanced clients (those that didn't match any original client)
    for enhanced_client in unmatched_enhanced:
        merged = enhanced_client.copy()
        # Ensure it has all required fields
        if 'rfc' not in merged or not merged['rfc']:
            merged['rfc'] = ''
        if 'c1' not in merged:
            merged['c1'] = 0.0
        if 'c2' not in merged:
...
def main():
    ...
    print("Merging client lists by grouping enhanced clients that match to originals...")
    ...
    angiui_clients = [c for c in merged_clients if 'ANGUI' in c['name']]
    ...
    # Show statistics
    matched_count = sum(1 for orig in original_clients if any(similarity(orig['name'], enh['name']) >= 50 for enh in enhanced_clients))
    print(f"Summary: {len(enhanced_clients)} enhanced + {len(original_clients)} original = {len(merged_clients)} merged clients")
    print(f"Original clients that had matches: {matched_count}/{len(original_clients)}")
