...
def merge_client_lists(original_clients: List[Dict], enhanced_clients: List[Dict]) -> List[Dict]:
    """Merge the two client lists based on normalized name matching"""
    # Create a dictionary for quick lookup of original clients by normalized name
    original_by_norm_name = {}
    for client in original_clients:
        norm_name = normalize_name(client['name'])
        original_by_norm_name[norm_name] = client

    # Create a dictionary for quick lookup of enhanced clients by normalized name
    enhanced_by_norm_name = {}
    for client in enhanced_clients:
        norm_name = normalize_name(client['name'])
        enhanced_by_norm_name[norm_name] = client

    # Get all unique normalized names
    all_norm_names = set(original_by_norm_name.keys()) | set(enhanced_by_norm_name.keys())

    merged_clients = []

    for norm_name in all_norm_names:
        original_client = original_by_norm_name.get(norm_name)
        enhanced_client = enhanced_by_norm_name.get(norm_name)

        # Start with enhanced client as base (for sheet information)
        if enhanced_client:
            merged = enhanced_client.copy()
        else:
            # If only in original, create a basic enhanced-like structure
            merged = {
                'name': original_client['name'],
                'num': original_client['num'],
        ...
        # Override with original client data for RFC and c1 (KM) and other fields if available
        if original_client:
...
def main():
    ...
    # Show some examples of merged clients
    print("\nExamples of merged clients:")
    for i, client in enumerate(merged_clients[:5]):
