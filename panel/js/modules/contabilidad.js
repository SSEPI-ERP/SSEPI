...
// DESCRIPCIÓN: Módulo de Contabilidad adaptado a Supabase
// BASADO EN: contabilidad-core.js original, reestructurado con servicios core
// FUNCIONALIDAD: Balance general, nómina, impuestos, bancos, cuadernillo PDF
// NOTA: La página pages/ssepi_contabilidad.html usa contabilidad-coi.js (SSEPI COI + motor local).
// ================================================
...
const ContabilidadModule = (function() {
    // ==================== ESTADO PRIVADO ====================
    let ventas = [];
    let compras = [];
    let nomina = [];
    let bancos = [];
    let empleados = [];
    let ingresosContabilidad = [];
    let facturas = [];

    let currentEmployee = null;
    let charts = { ingEgr: null, egresos: null };
    let dateRange = { start: null, end: null, type: 'month', refDate: null };
    let filterType = 'all'; // para tabla de movimientos
    ...
    const empleadosService = createDataService('empleados');
    ...
    // ==================== INICIALIZACIÓN ====================
    async function init() {
        console.log('✅ [Contabilidad] Conectado');
        await _initUI();
        _bindEvents();
    ...
    async function _loadVentas() {
        ventas = await ventasService.select({ estatus_pago: 'Pagado' });
    }
    ...
    async function _loadNomina() {
        nomina = await nominaService.select({}, { orderBy: 'fecha_pago', ascending: false });
    }

    async function _loadEmpleados() {
        empleados = await empleadosService.select({ activo: true });
        _updateEmployeesList();
    }

    async function _loadBancos() {
        bancos = await bancosService.select({}, { orderBy: 'fecha', ascending: false });
        _updateBankMovementsUI();
        _updateSaldoTotal();
    }

    async function _loadIngresos() {
    ...
    function _setupRealtime() {
        ...
        const subVentas = supabase
            .channel('contabilidad_ventas')
            .on('postgres_changes', { event: '*', schema: 'public', table: 'ventas' }, payload => { _loadVentas(); _refreshBalanceView(); })
            .subscribe();
        subscriptions.push(subVentas);

        const subCompras = supabase
            .channel('contabilidad_compras')
            .on('postgres_changes', { event: '*', schema: 'public', table: 'compras' }, payload => { _loadCompras(); _refreshBalanceView(); })
            .subscribe();
        subscriptions.push(subCompras);

        const subNomina = supabase
            .channel('contabilidad_nomina')
            .on('postgres_changes', { event: '*', schema: 'public', table: 'pagos_nomina' }, payload => { _loadNomina(); _refreshBalanceView(); })
            .subscribe();
        subscriptions.push(subNomina);

        const subBancos = supabase
            .channel('contabilidad_bancos')
            .on('postgres_changes', { event: '*', schema: 'public', table: 'movimientos_banco' }, payload => { _loadBancos(); _refreshBalanceView(); })
            .subscribe();
        subscriptions.push(subBancos);

        const subEmpleados = supabase
            .channel('contabilidad_empleados')
    ...
    function _refreshBalanceView() {
        const ventasFiltradas = ventas.filter(v => {
            ...
            return f && f >= dateRange.start && f <= dateRange.end;
        });
        const comprasFiltradas = compras.filter(c => {
            const f = c.fecha_creacion ? _parseDate(c.fecha_creacion) : null;
            return f && f >= dateRange.start && f <= dateRange.end;
        });
        ...
        const ingresosFiltrados = ingresosContabilidad.filter(i => {
            const f = i.fecha_pago ? _parseDate(i.fecha_pago) : null;
            return f && f >= dateRange.start && f <= dateRange.end;
        });
        ...
        let ingresosContab = ingresosFiltrados.reduce((acc, i) => acc + (i.monto_total || 0), 0);
        let totalIngresos = ingresosVentas + ingresosBancos + ingresosContab;
        ...
        _updateMovementsTable(ventasFiltradas, comprasFiltradas, nominaFiltrada, bancosFiltrados, ingresosFiltrados);
        _updateChartsWithRange();
    }

    function _updateMovementsTable(ventas, compras, nominas, bancos, ingresos) {
        const tbody = document.getElementById('movementsTableBody');
        let movimientos = [];

        ventas.forEach(v => movimientos.push({ fecha: v.fecha, concepto: `Venta: ${v.folio || v.id}`, origen: 'Ventas', tipo: 'Ingreso', monto: v.total || 0 }));
        compras.forEach(c => movimientos.push({ fecha: c.fecha_creacion, concepto: `Compra: ${c.folio || c.id}`, origen: 'Compras', tipo: 'Egreso', monto: (c.total || 0) * -1 }));
        nominas.forEach(n => movimientos.push({ fecha: n.fecha_pago, concepto: `Nómina: ${n.empleado_nombre}`, origen: 'Nómina', tipo: 'Egreso', monto: (n.total || 0) * -1 }));
        bancos.forEach(b => movimientos.push({ fecha: b.fecha, concepto: b.concepto, origen: 'Bancos', tipo: b.tipo === 'ingreso' ? 'Ingreso' : 'Egreso', monto: b.tipo === 'ingreso' ? (b.monto || 0) : -(b.monto || 0) }));
        ingresos.forEach(i => movimientos.push({ fecha: i.fecha_pago, concepto: `Ingreso contable: ${i.folio}`, origen: 'Contabilidad', tipo: 'Ingreso', monto: i.monto_total || 0 }));

        movimientos.sort((a,b) => _parseDate(b.fecha) - _parseDate(a.fecha));

        if (filterType === 'ingresos') movimientos = movimientos.filter(m => m.tipo === 'Ingreso');
        else if (filterType === 'egresos') movimientos = movimientos.filter(m => m.tipo === 'Egreso');

        if (movimientos.length === 0) {
            tbody.innerHTML = '<tr><td colspan="5" class="text-center" style="padding:40px;"><i class="fa-solid fa-inbox" style="font-size:48px; opacity:0.3;"></i><br>No hay movimientos en este período</td></tr>';
            return;
        }
        ...
        movimientos.forEach(m => {
            ...
            html += `<tr><td>${fecha}</td><td>${m.concepto}</td><td>${m.origen}</td><td><span class="movement-type ${tipoClass}">${m.tipo}</span></td><td class="movement-amount ${montoClass}">${_formatMoney(Math.abs(m.monto))}</td></tr>`;
        });
        tbody.innerHTML = html;
    }

    function _updateChartsWithRange() {
        _initCharts();
    }

    function _initCharts() {
        if (charts.ingEgr) charts.ingEgr.destroy();
        if (charts.egresos) charts.egresos.destroy();
        ...
        // Datos de ejemplo (en producción se calcularían con datos reales)
        charts.ingEgr = new Chart(ctx1, {
            type: 'bar',
            data: {
                labels: ['Ene','Feb','Mar','Abr','May','Jun'],
                datasets: [
                    { label: 'Ingresos', data: [120000,150000,180000,140000,200000,220000], backgroundColor: isDark ? 'rgba(129,199,132,0.8)' : 'rgba(0,82,204,0.8)', borderColor: isDark ? '#81c784' : '#0052cc', borderWidth: 1 },
        ...
        charts.egresos = new Chart(ctx2, {
            type: 'doughnut',
            data: {
                labels: ['Materiales','Servicios','Nómina','Otros'],
                datasets: [{
                    data: [40,25,25,10],
    ...
    function _updateChartPeriod(e) {
        ...
        if (charts.ingEgr) {
            ...
            charts.ingEgr.data.labels = newData.labels;
            charts.ingEgr.data.datasets[0].data = newData.ingresos;
            charts.ingEgr.data.datasets[1].data = newData.egresos;
            charts.ingEgr.update();
        }
    }
    ...
    function _updateEmployeesList() {
        ...
        if (empleados.length === 0) {
            container.innerHTML = '<div class="empty-state" style="padding:40px 20px;"><i class="fa-solid fa-user" style="font-size:48px; opacity:0.3;"></i><h3>No hay empleados registrados</h3><p>Agrega el primer empleado haciendo clic en "Nuevo"</p></div>';
            return;
        }
        let html = '';
    ...
    function _selectEmployee(id) {
        currentEmployee = empleados.find(e => e.id === id);
        ...
        const initials = currentEmployee.nombre.split(' ').map(n => n[0]).join('').toUpperCase().substring(0,2);
        document.getElementById('selectedEmployeeInfo').innerHTML = `
            <div class="employee-avatar">${initials}</div>
            <div style="flex:1;">
                <div style="font-weight:800; color:var(--text-main);">${currentEmployee.nombre}</div>
                <div style="font-size:13px; color:var(--text-muted);">${currentEmployee.puesto}</div>
                <div style="font-size:12px; color:var(--c-contabilidad); font-weight:700; margin-top:5px;">Sueldo diario: $${currentEmployee.sueldo_diario.toFixed(2)}</div>
            </div>
        `;
        _generateDaysGrid();
        _calculateNomina();
    ...
    function _calculateNomina() {
        ...
        const sueldoDiario = currentEmployee.sueldo_diario;
    ...
    async function _saveNominaPayment() {
        ...
        const pagoData = {
            ...
            empleado_nombre: currentEmployee.nombre,
        ...
        try {
            ...
            // Encolar para COI (nómina)
            enqueueCoiJob({
                erp_source: 'nomina',
                erp_id: String(inserted?.id || pagoData.referencia || Date.now()),
                folio: pagoData.referencia || null,
                idempotency_key: `nomina:${inserted?.id || pagoData.referencia || ''}`,
    ...
    async function _loadNominaHistory() {
        ...
        const historial = nomina.slice(0,20);
    ...
    async function _calculateIVA(period) {
        ...
        ventasPeriodo.forEach(v => ventasBase += (v.total || 0) / 1.16);
        const comprasPeriodo = compras.filter(c => {
            const f = c.fecha_creacion ? new Date(c.fecha_creacion) : null;
            return f && f >= start && f <= end;
        });
        comprasPeriodo.forEach(c => comprasBase += (c.subtotal || c.total || 0) / 1.16);
    ...
    function _updateBankMovementsUI(filter = 'all') {
        ...
        let filtered = bancos;
        if (filter === 'ingreso') filtered = bancos.filter(m => m.tipo === 'ingreso');
        else if (filter === 'egreso') filtered = bancos.filter(m => m.tipo === 'egreso');
    ...
    function _updateSaldoTotal() {
        let saldo = 0;
        bancos.forEach(m => { if (m.tipo === 'ingreso') saldo += m.monto; else saldo -= m.monto; });
    ...
    async function _saveBankMovement() {
        ...
        const mov = {
            ...
            creado_por: (await authService.getCurrentProfile())?.id
        };
        const csrfToken = sessionStorage.getItem('csrfToken');
        try {
            const inserted = await bancosService.insert(mov, csrfToken);
            enqueueCoiJob({
                erp_source: 'bancos',
                erp_id: String(inserted?.id || Date.now()),
                folio: inserted?.id ? String(inserted.id).slice(-6) : null,
                idempotency_key: `bancos:${inserted?.id || ''}`,
                payload_json: { ...mov, id: inserted?.id },
            }).then(r => {
    ...
    function _generarPDFSeleccionado() {
        ...
        const VERDE = '#0052cc', DORADO = '#c49a6c', BLANCO = '#ffffff', HEADER_HEIGHT = 108;
        ...
        function agregarReporteNomina(doc) {
            ...
            let nominaData = nomina.slice(0,10).map(n => [n.empleado_nombre, `${n.periodo_inicio} al ${n.periodo_fin}`, n.dias_trabajados + ' días', _formatMoney(n.total).replace('$','')]);
        ...
        function agregarFlujoEfectivo(doc) {
            ...
            bancos.forEach(m => { if (m.tipo === 'ingreso') ingresosBank += m.monto; else egresosBank += m.monto; });
    ...
    function _bindEvents() {
        ...
        document.getElementById('periodType').addEventListener('change', _applyDateFilter);
        document.querySelectorAll('#tabBalance .filter-btn').forEach(btn => {
            btn.addEventListener('click', (e) => {
                document.querySelectorAll('#tabBalance .filter-btn').forEach(b => b.classList.remove('active'));
        ...
        document.querySelectorAll('#tabImpuestos .period-btn').forEach(btn => {
            btn.addEventListener('click', (e) => {
                document.querySelectorAll('#tabImpuestos .period-btn').forEach(b => b.classList.remove('active'));
                e.target.classList.add('active');
                _calculateIVA(e.target.dataset.ivaPeriod);
            });
        });
        ...
        document.querySelectorAll('[data-bank-filter]').forEach(btn => {
            btn.addEventListener('click', (e) => {
                ...
                document.querySelectorAll('[data-bank-filter]').forEach(b => {
                    ...
                    b.style.color = 'var(--c-contabilidad)';
    ...
    function _openTab(tabId) {
        ...
        const btn = document.getElementById(`tab${tabId.replace('tab', '')}Btn`);
        if (btn) btn.classList.add('active');
        if (tabId === 'tabNomina') {
            _loadNominaHistory();
        }
        if (tabId === 'tabImpuestos') {
            _calculateIVA('month');
        }
    }
